    ...
    <Card>
        ...
        <div class="field-list">
            ...
            <div class="field-item-panel">
                ...
                <div
                    class="sale-setting-detail-table"
...
export default {
                        ...
                        render: (h, params) => {
                            return h('div', [
                                h('input', {
                                    ...
                                    attrs: {
                                        disabled: this.detail[params.index].status
                                    },
                                    on: {
                                        change: (res) => {
                                            this.detail[params.index].price = res.target.value;
                        ...
                        render: (h, params) => {
                            return h('div', [
                                h('input', {
                                    ...
                                    on: {
                                        change: (res) => {
                                            this.detail[params.index].header = res.target.value;
                                            // console.log(JSON.stringify(params));
                                            console.log(params.index);
                                        }
                                    }
                        ...
                        render: (h, params) => {
                            return h('div', [
                                h('a', {
                                    on: {
                                        click: () => {
                                            if (this.have_guige == 1 && this.detail[params.index]) {
                                                this.detail[params.index].price = 0;
                                                this.detail[params.index].status = !this.detail[params.index].status;
                                            } else {
                                                this.$Message.info("该商品没有规格")
                                            }
                                        }
                                    }
                                }, this.detail[params.index] && this.detail[params.index].status ? '取消设置' : '设置')
                            ]);
                        }
                    }
                ],
    ...
    methods: {
        ...
        get: function () {
            var _this = this;
            var params = {
                action: 'groupbuy_get_goods_group_buy',   //$goodsid=商品ID,$group_buy_id=团ID
                appid: this.vueAppid,
            ...
            this.$http.post("/api_edit.php", this.$qs.stringify(params)).then(function (res) {
                ...
                _this.numInfo(res.data.body.num_of_people);
                _this.addDetail(res.data.body.num_of_people, res.data.body.guige);
        ...
        numInfo: function (data) {
            ...
            this.infoData.data = arr;
        },
        addDetail: function (params, guige) {
            console.log(params);
            var arr = [];
            for (var i = 0; i < params.length; i++) {
                if (guige) {
                    for (var k = 0; k < guige.length; k++) {
                        arr.push({
                            goods_guige_id: guige[k].id,
                            num_of_people: params[i],
                            leader_price: 0,
                            normal_price: 0
                        });
                    }
                            ...
                            render: (h, params) => {
                                return h('div', [
                                    h('input', {
                                        ...
                                        on: {
                                            change: (res) => {
                                                this.detail[params.column.indexOf].goods_guige[params.index].normal_price = res.target.value;
                                                console.log(this.detail[params.column.indexOf].goods_guige[params.index]);
                                                // this.detail[params.index].price = res.target.value;
                                                // console.log(this.detail[params.index].price);
        ...
        handleSubmit: function () {
            ...
            for (var i = 0; i < this.detail.length; i++) {
                for (var j = 0; j < this.detail[i].goods_guige.length; j++) {
                    var s = this.detail[i].goods_guige[j];
                    this.detail[i].goods_guige[j].leader_price = this.header == 0 ? s.normal_price : (this.header == 1 ? this.detail[i].header : 0)
                }
                arr.push({
                    num_of_people: this.detail[i].num,
                    leader_price: this.header == 0 ? this.detail[i].price : (this.header == 1 ? this.detail[i].header : 0),
